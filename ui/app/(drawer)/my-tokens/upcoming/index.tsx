...
export default function UpcomingTokensScreen() {
  ...
  const navigateToDoctorDetails = (doctorId: number) => {
    router.push({
      pathname: `/(drawer)/dashboard/doctor-details/${doctorId}` as any,
      params: { from: '(drawer)/my-tokens/'}
    });
  };
  ...
  return (
    <ThemedView style={tw`flex-1`}>
      ...
      >
        <View
          style={tw`bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md mb-6`}
        >
          ...
          {isLoading ? (
            <View style={tw`items-center justify-center py-8`}>
              ...
              <MyText style={tw`mt-4 text-gray-500`}>
                Loading your tokens...
              </MyText>
            </View>
          ) : isError ? (
            ...
            <View
              style={tw`mt-4 p-6 bg-gray-100 dark:bg-gray-700 rounded-lg items-center`}
            >
              <MyText
                style={tw`text-center text-gray-500 dark:text-gray-400 mb-2`}
              >
...
const TokenCard: React.FC<TokenCardProps> = ({ token, onDoctorPress }) => {
  ...
  return (
    <View
      style={tw`bg-white dark:bg-gray-700 p-4 rounded-xl shadow-sm mb-4 border-l-4 ${
    ...
    >
      <View style={tw`flex-row justify-between items-start mb-2`}>
        ...
        <View style={tw`bg-blue-100 dark:bg-blue-900 px-3 py-1 rounded-full`}>
          <MyText
            style={tw`text-blue-800 dark:text-blue-200 text-xs font-medium`}
          >
            #{token.queueNumber}
          </MyText>
        </View>
      ...
      {token.description && (
        <View style={tw`mt-2 p-2 bg-gray-50 dark:bg-gray-800 rounded`}>
          <MyText style={tw`text-sm text-gray-600 dark:text-gray-300`}>
            {token.description}
          </MyText>
        </View>
      )}
